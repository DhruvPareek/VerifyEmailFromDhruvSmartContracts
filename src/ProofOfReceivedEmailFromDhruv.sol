...
contract ProofOfReceivedEmailFromDhruv is ERC721Enumerable {
    ...
    uint32 public constant targetEmailAddressIndexInSignals = 1; // index of first packed twitter username in signals array
    uint32 public constant usernameLengthInSignals = 1; // length of packed twitter username in signals array
    uint32 public constant senderAddressIndexInSignals = 2; // index of ethereum address in signals array
    uint32 public constant ethereumAddressIndexInSignals = 3; // index of ethereum address in signals array
    ...
    //     string[2] memory domainStrings = ["verify@x.com", "dhruvpareek883@gmail.com"];
    //     return
    ...
    /// @param signals Public signals of the circuit. First item is pubkey_hash, second item is email address, the last one is etherum address
    function mint(uint256[8] memory proof, uint256[4] memory signals) public returns (bool){
        ...
        require(signals[senderAddressIndexInSignals] == 0x00000000000000006d6f632e6c69616d67403338386b65657261707675726864, "invalid signals");
        ...
        for (uint256 i = targetEmailAddressIndexInSignals; i < (targetEmailAddressIndexInSignals + usernameLengthInSignals); i++) {
        ...
        if(emailAddresstoEthereumAddress[messageBytes] != address(0)){
            return false;
        }
        emailAddresstoEthereumAddress[messageBytes] = msg.sender;
        return true;
    }
